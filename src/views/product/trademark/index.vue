<template>
    <el-card class="box-card">
        ...
        -->
        <el-table style="margin:10px 0px;" fit border :data="trademarkArr">
            ...
            <el-table-column label="品牌LOGO" align="center">
                <!-- 作用域插槽 -->
                <template #="{ row }">
                    <img :src="row.logoUrl" alt="(图片缺失)" style="width: 60px;height: 60px;">
                </template>
            </el-table-column>
            <el-table-column label="品牌操作" align="center">
                ...
                <template #="{ row }">
                    ...
                    <el-popconfirm width="200px" :title="`您确定要删除'${row.tmName}'吗？`" icon="Delete"
    ...
    <el-dialog v-model="dialogFormVisible" :title="trademarkParams.id ? '修改品牌' : '添加品牌'">
        <el-form style="width:80%;" :model="trademarkParams" :rules="rules" ref="formRef">
            ...
            <el-form-item label="品牌LOGO" label-width="100px" prop="logoUrl">
                ...
                <el-upload class="avatar-uploader" list-type="picture-card" action="/api/admin/product/fileUpload"
...
const getHasTrademark = async (page = 1) => { //默认参数，回到第一页
    pageNo.value = page
...
const validatorTmName = (_rule: any, value: any, callback: any) => {
    ...
    if (value.trim().length >= 2) {
        ...
        //校验未通过返回的错误的提示信息
        callback(new Error('品牌名称位数须大于等于两位'))
    }
}
// 品牌LOGO图片的自定义校验规则方法
// value 参数来自于当前正在验证的表单项的绑定值
...
const removeTrademark = async (id: number) => {
    ...
    if (result.code == 200) {
        ...
        // 再次获得品牌列表
        getHasTrademark(trademarkArr.value.length > 0 ? pageNo.value : pageNo.value - 1)
    } else {
        ElMessage({
            type: 'error',
            message: '删除品牌失败'
