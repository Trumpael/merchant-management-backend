<template>
    <el-form label-width="100px">
        ...
        <el-form-item label="SPU销售属性">
            ...
            <el-button @click="addSaleAttr" :disabled="saleAttrIdAndValueName ? false : true" style="margin-top:5px"
                type="primary" size="default" icon="Plus">添加属性</el-button>
            <!-- table展示销售属性与属性值的地方 -->
            <el-table border style="margin:10px 0px" :data="saleAttr">
                <el-table-column label="序号" type="index" align="center" width="80px"></el-table-column>
                <el-table-column label="销售属性名字" width="120px" align="center" prop="saleAttrName"></el-table-column>
                <el-table-column label="销售属性值" align="center">
                    ...
                    <template #="{ row }">
                        ...
                        <el-input ref="inputRef" @blur="toLook(row)" v-model="row.saleAttrValue" v-if="row.flag == true"
...
const initHasSpuData = async (spu: SpuData) => {
    ...
    try {
        ...
        if (result.code !== 200) {
            throw new Error('获取品牌数据失败');
        }
        AllTradeMark.value = result.data;
        // 获取某一个品牌旗下全部售卖商品的图片
        let result1: SpuHasImg = await reqSpuImageList((spu.id as number));
        if (result1.code !== 200) {
            throw new Error('获取商品图片失败');
        }
        ...
        if (result2.code !== 200) {
            throw new Error('获取销售属性失败');
        }
        saleAttr.value = result2.data;
        // 获取整个项目全部 SPU 的销售属性
        let result3: HasSaleAttrResponseData = await reqAllSaleAttr();
        if (result3.code !== 200) {
            throw new Error('获取全部销售属性失败');
        }
