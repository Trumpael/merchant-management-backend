<template>
    ...
    <el-drawer v-model="isDrawerVisible1">
        <template #header>
            <h4>分配角色(职位)</h4>
        </template>
        <template #default>
        ...
        <template #footer>
            <div style="flex: auto">
                <el-button @click="isDrawerVisible1 = false">取消</el-button>
...
//定义响应式数据控制抽屉的显示与隐藏
let isDrawerVisible = ref<boolean>(false);
//控制分配角色抽屉显示与隐藏
let isDrawerVisible1 = ref<boolean>(false);
//存储全部职位的数据
...
watch(isDrawerVisible, (newValue, _oldValue) => {
  if (!newValue) {
    // 当抽屉关闭时清空数据
...
</style>_oldValue_rule_rule_rule_$event_$event_$event_$event_oldValue_rule_rule_rule_$event_$event_$event_$event_rule_rule_rule_$event_$event_$event_$event